...
\item C'est un dérivé de Debian : de ce fait, un administrateur sachant
  configurer un serveur sous Debian pourra facilitement s'adapter aux serveur sous
  Ubuntu.

\item Il vise le grand public. De ce fait, est beaucoup moins utilisé dans le
...
Afin de posséder assez d'espace de stockage, deux disques de 20 Go en RAID 1 ont
été utililisés. De surcroît, le partionnement du disque a été réalisé avec un
partition racine, \textit{/boot}, et un groupe de volume LVM nommé
\textit{VolGroup}.
...
Afin d'éviter de perturber l'accès au serveur, chaque jour à 2h du matin, une
...
Quant à la sauvegarde différentielle, celle-ci s'opère uniquement le dimanche
à 2h du matin.
...
\underline{Remarque :} à défault d'utiliser \textit{fcron} n'étant plus
