...
iptables -A INPUT -s 10.1.0.0/16 -d 10.1.0.0/16 -p udp -m multiport --dport 137,138 -m state --state NEW,ESTABLISHED -j ACCEPT
iptables -A INPUT -s 10.1.0.0/16 -d 10.1.0.0/16 -p tcp -m multiport --dport 139,445 -m state --state NEW,ESTABLISHED -j ACCEPT

# Allow outgoing Samba (UDP: 137,138 | TCP: 139,445)
iptables -A OUTPUT -d 10.1.0.0/16 -p udp -m multiport --dport 137,138 -m state --state ESTABLISHED -j ACCEPT
iptables -A OUTPUT -d 10.1.0.0/16 -p tcp -m multiport --dport 139,445 -m state --state ESTABLISHED -j ACCEPT
